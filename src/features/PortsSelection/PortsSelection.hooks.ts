...
export const usePortsSelection = () => {
  ...
  const handlePortChange = useCallback(
    (location: 'origin' | 'destination') => (e: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
      dispatch(changePort({ location, value: e.target.value }));
    },
    [dispatch],
  );

  const options = useMemo(() => availablePorts.map(mapAvailablePortsToOptions), [
    mapAvailablePortsToOptions,
    availablePorts,
  ]);
